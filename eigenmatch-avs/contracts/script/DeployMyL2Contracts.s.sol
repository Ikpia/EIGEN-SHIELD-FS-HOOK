...
import {IEigenShieldHook} from "@project/interfaces/IEigenMatchHook.sol";
...
contract DeployMyL2Contracts is Script {
    ...
    function run(string memory environment, string memory _context) public {
        ...
        address hookAddress = vm.envAddress("EIGENSHIELD_HOOK_ADDRESS");
        EigenShieldDigestRegistry registry =
            EigenShieldDigestRegistry(_readCustomL1Address(environment, "EigenShieldDigestRegistry"));

        address[] memory watchers = new address[](1);
        watchers[0] = context.avs;

        EigenShieldAttestationRelay executor =
            new EigenShieldAttestationRelay(IEigenShieldHook(hookAddress), registry, context.avs, watchers, 1);
        console.log("EigenShieldAttestationRelay deployed to:", address(executor));
        ...
        outputs[0] = Output({name: "EigenShieldAttestationRelay", contractAddress: address(executor)});
    ...
    function _readCustomL1Address(string memory environment, string memory key) internal view returns (address) {
        string memory customL1File = string.concat("script/", environment, "/output/deploy_custom_contracts_l1_output.json");
        string memory json = vm.readFile(customL1File);
