...
This directory contains the EigenLayer AVS that underpins the **EigenMatch CoW Hook** described in `README.md` at the repo root. Operators run this AVS to ingest intent bundles coming out of EigenCompute TEEs, validate the hardware attestation, and drive the on-chain Uniswap v4 hook so matched volume settles internally before touching AMM liquidity.

The project originally came from the Hourglass template, but we have aligned it with the EigenMatch system design:

| Component | EigenMatch responsibility |
|-----------|---------------------------|
| `cmd/` performer | Validates EigenCompute settlement bundles (docker digest + TEE measurement allowlists, replay salt, pool allowlist) and emits verified bundle responses for the executor bridge. |
| `contracts/` | Placeholder for EigenMatch-specific registrars/hooks that will be swapped in once the settlement executor contracts are finalized. |
| `.devkit/` | Deployment scripts that `devkit avs devnet start` invokes. |
| `config/` | Project metadata (`config.yaml`) plus context files (`config/contexts/*.yaml`) so DevKit can run from this directory as required by the EigenCloud docs. |
...
All of those steps map back to `docs/system-design.md` and `docs/prd.md`; changes in this folder should mirror those documents.

## Running DevKit commands (best practice)

The EigenCloud documentation is explicit: every `devkit avs …` command must be executed from the AVS project root (the directory that contains `config/config.yaml`). We follow that rule by keeping all DevKit work inside `avs/eigenmatch-avs` and by updating the parent `Makefile` to call the CLI from here.

```bash
# 0. One-time: copy config.template.yaml -> config/config.yaml
make -C avs bootstrap

# 1. Generate or refresh the EigenMatch + devnet contexts
make -C avs context

# 2. Start the devnet (deploys contracts + Hourglass infra)
...
Those make targets wrap:
- `devkit avs context create --context eigenmatch …`
- `devkit avs devnet start --context devnet`
- `devkit avs devnet stop --context devnet`

You can also run the commands manually as long as you `cd avs/eigenmatch-avs` first.

### Configuration files

```
config/
  config.yaml                  # project metadata (name, version, default context)
  contexts/
    devnet.yaml                # local Anvil devnet (forked from Sepolia / OP Sepolia)
    eigenmatch.yaml            # shared context that uses the Alchemy RPCs + EigenMatch allowlists
env.example                    # helper for L1/L2 fork URLs and matcher allowlists
```

Copy `env.example` to `.env` and update it with your Alchemy keys. DevKit automatically prioritises the environment variables over the values in the YAML files, matching the EigenCloud recommendation from `context/eigencloud-docs/static/devkit.md`.

## Implementing EigenShield (Flow Sentinel) logic

### Performer (`cmd/main.go`)

The performer implements two phases:

1. `ValidateTask` – decode the task payload into a verdict bundle, check docker digest and TEE measurement allowlists (`ALLOWED_DOCKER_DIGESTS`, `ALLOWED_TEE_MEASUREMENTS`), ensure the bundle is still fresh (`maxDelay` env), enforce replay protection using the `replaySalt`, and verify at least one verdict exists before handing anything to the executor bridge.
...
The template ships with Hourglass sample contracts (`TaskAVSRegistrar`, `AVSTaskHook`, etc.) so DevKit can compile/deploy during devnet boot. We layer EigenShield-specific glue on top:

- `l1-contracts/EigenMatchDigestRegistry.sol` (contract `EigenShieldDigestRegistry`) – publishes the EigenCompute/EigenAI docker digest + TEE measurement allowlist that operators/watchers reference before accepting a detection bundle.
- `l2-contracts/EigenMatchSettlementExecutor.sol` (contract `EigenShieldAttestationRelay`) – verifies watcher signatures and registry state before calling `IEigenShieldHook.submitVerdictBundle`.

These sit alongside the Hourglass infra (registrar, task hook) so devnet spins up end-to-end while still reflecting the sandwich-detection requirements in `README.md`.

## What’s left to align with the root README?

- Port the real EigenShield detection contracts into `contracts/src/**`.
- Update `.devkit/scripts/deploy*.s.sol` to emit JSON artifacts consumed by the performer (hook addresses, fee configs, etc.).
- Wire the watcher/executor bridge so validated verdict bundles automatically hit the Uniswap hook rather than just being logged.
- Expand Go/Forge tests (`cmd/main_test.go`, `contracts/test/*.t.sol`) to cover the EigenShield-specific invariants listed in `docs/prd.md`.

When working in this directory always cross-reference:
- `README.md` (project pitch and success metrics)
- `docs/prd.md` (requirements & milestones)
- `docs/system-design.md` (data flow, trust boundaries, replay semantics)
- `context/eigencompute-intent-engine.md` (TEE + matcher expectations)

Those documents are the single source of truth for how this AVS should behave.
